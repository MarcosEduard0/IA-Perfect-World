...
def main(): 
    ...
    while True:
        Quant = int(pyautogui.password(text='Quantos adds de intervalo você quer?\n', title='I.A do John', mask=None))
        if Quant < 1 or Quant > 4:
            pyautogui.alert('Por favor digite um valor entre 1 e 4.','I.A do John','Ok')
        else:
            break
    
    QuantRolete = int(pyautogui.password(text='Quantas Itens de Reforjar você possui?\n', title='I.A do John', mask=None))
    ...
    while(finalizador <= rolete):
        ...
        intervalo = adds.count('Intervalo') 
        ataque = adds.count('Ataque')
        #print(destreza)

        if intervalo >= Quant or ataque >= Quant:
            pyautogui.click(BotaoReproduzir[0]+155,BotaoReproduzir[1]-15) #CLica em segurar o novo add
            pyautogui.alert('Parabéns, conseguimos uma parte do set com '+ str(ataque) +' add de intervalo.','I.A do John','Obrigado')
